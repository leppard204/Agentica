...
export async function analyzePromptAI(prompt: string) {
...
export async function chatbotHandler(prompt: string) {
  ...
  switch (intent) {
    case 'register_project':
      if (!extracted_params.description) throw new Error('description is required');
      return await createProject({ name: extracted_params.description, description: extracted_params.description, industry: '' });

    case 'initial_email':
      if (!extracted_params.projectId || !extracted_params.leads) throw new Error('projectId and leads are required');
    ...
    case 'email_rewrite_request':
      ...
      if (!originalEmail) throw new Error('Original email not found for rewriting');
    ...
    case 'connect_leads':
      if (!extracted_params.projectId) throw new Error('projectId is required');
